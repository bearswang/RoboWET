function [R0,t0,w0,a0,c0,Q0,feasible]= feasible_point_pursuit(N, M, h, g, Er, Ec, E, Pc, Pr, Pu, Pz, miu, T)
% moving parameters
EM=90.54;
s=[1.5 3 1.5 0];
...
[R0,t0,w0,a0,c0,Q0,feasible]= init(N, M, h, g, Er, Ec, E, Pc, Pr, Pu, Pz, miu, T);
...
for iter=1:ITER_MAX
   % For future use
    ...
    Phi_term3=zeros(M,1);
    for m=1:M
        Phi_term1(m)=-t0(m)*log2(t0(m)*Pr./(Q0(1,m)+Q0(2,m))+1);
        Phi_term2(m)=1./log(2)*t0(m)^2./(t0(m)*(Q0(1,m)+Q0(2,m))+(Q0(1,m)+Q0(2,m))^2./Pr);
        Phi_term3(m)=-log2(t0(m)*Pr./(Q0(1,m)+Q0(2,m))+1)-t0(m)/log(2)./(t0(m)+(Q0(1,m)+Q0(2,m))./Pr);
    end
    ...
    maximize R-10^6*sum(sum(varphi))
    subject to
    ...
    for i=1:2
        for m=1:M
            temp=0;
            for l=1:m
                H=-H_term1*H_term2*t(l)+H_term1*t(l)+H_term1*H_term2*exp(nu)*H_term3(i,l);
                temp=temp+b(i,l)+H+(2*t(l)+s(l))*Pc;
            end
            temp<=E(i)+varphi(i,m); % (line 5)
        end
    end
